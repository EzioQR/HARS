...
public class RWSwitchedBasedSearch extends DistanceBasedSearch{
    public RWSwitchedBasedSearch(Config config, VM vm) {
        super(config, vm);
    }
    ...
    protected void sortrule() {
        ...
        System.out.print("RWswitchtimes=:");
        for (int i = 0; i <subqueue.size() ; i++) {
            System.out.print(subqueue.get(i).getRWswitchtimes()+" ");
        }
        System.out.println();
        ...
        for (int i = 0; i <subqueue.size() ; i++) {
            if (i==0)
                current=subqueue.get(i).getRWswitchtimes();
            if (subqueue.get(i).getRWswitchtimes()<current)
            {
                finalindex=i;
                break;
            }
