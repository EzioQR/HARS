...
public class SharedVariableNumberBasedSearch extends DistanceBasedSearch{


    public SharedVariableNumberBasedSearch(Config config, VM vm) {
        super(config, vm);
    }
    ...
    protected void sortrule() {
        ...
        System.out.print("sharedVariableNumbers=:");
        for (int i = 0; i <subqueue.size() ; i++) {
            System.out.print(subqueue.get(i).getsharedvarnumbers()+" ");
        }
        System.out.println();
        ...
        for (int i = 0; i <subqueue.size() ; i++) {
            if (i==0)
                current=subqueue.get(i).getsharedvarnumbers();
            if (subqueue.get(i).getsharedvarnumbers()<current)
            {
                finalindex=i;
                break;
            }
